console.log("Custom JavaScript loaded for Textile RnD Hanger List");

frappe.listview_settings['Textile RnD Hanger'] = {
...
function add_scan_button_beside_id_filter(listview) {
    ...
    setTimeout(() => {
        // Get the filter for the 'name' field
        let name_filter = listview.filter_area.standard_filters_wrapper.find('div[data-fieldname="name"]');
        ...
        if (name_filter.length > 0 && !name_filter.find('.scan-btn').length) {
            ...
            // Insert the button next to the name filter input
        ...
        fields: [
            {
                ...
                options: `<div id="${scannerId}" style="width:100%;"></div>`
            }
        ],
    ...
    setTimeout(() => {
        ...
        html5QrCode.start(
            ...
            (decodedText, decodedResult) => {
                ...
                // Set the scanned value in the name filter input
                let name_filter = listview.filter_area.standard_filters_wrapper.find('div[data-fieldname="name"]');
                let name_filter_input = name_filter.find('input');
